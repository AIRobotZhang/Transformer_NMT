...
def train_model(model, train_iter, dev_iter, epoch, lr, loss_func, eos_id, index2tgtword):
    ...
    for idx, batch in enumerate(train_iter):
        ...
        pred, _, _, _ = model(src_text, tgt_text[:, :-1])
        ...
        if idx % 10 == 0:
            logger.info('Epoch:%d, Idx:%d, Training Loss:%.4f', epoch, idx, loss_.item())
...
def eval_model(model, val_iter, loss_func, eos_id, index2tgtword):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(val_iter)):
            ...
            score += metrics.bleu(tgt_text[:, 1:], pred_id, tgt_origin_lens-1, eos_id)
            ind += 1
...
def main():
    ...
    src_pad_id = dict(src_TEXT.vocab.stoi)['<pad>']
    tgt_pad_id = dict(tgt_TEXT.vocab.stoi)['<pad>']
    index2tgtword = tgt_TEXT.vocab.itos

    model = Transformer(src_vocab_size, args.src_max_len, tgt_vocab_size, args.tgt_max_len, src_word_embeddings, src_pad_id, bos_id, \
                 tgt_word_embeddings, tgt_pad_id, args.num_layers, args.model_dim, args.num_heads, args.ffn_dim, args.dropout)
